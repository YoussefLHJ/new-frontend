...
@Component({
  selector: 'app-tournee-releve-detail-create-admin',
  standalone: false,
  templateUrl: './tournee-releve-detail-create-admin.component.html'
})
export class TourneeReleveDetailCreateAdminComponent  implements OnInit {
    ...
    validateTourneeReleveCompteurs(){
    ...
    const isPermistted = await this.roleService.isPermitted('TourneeLotReleve', 'add');
    if(isPermistted) {
    ...
    const isPermistted = await this.roleService.isPermitted('LotReleve', 'add');
    if(isPermistted) {
