...
@Component({
    selector: 'app-new-releve-edit-admin',
    standalone: false,
    templateUrl: './new-releve-edit-admin.component.html'
})
export class NewReleveEditAdminComponent implements OnInit {
    ...
    edit(): void {
        ...
        if (this.errorMessages.length === 0) {
            this.service.edit().subscribe({
                next: result => {
                    const myIndex = this.service.items.findIndex(e => e.id === this.item.id);
                    this.service.items[myIndex] = result;
    ...
    async openCreateUniteReleve() {
        const isPermitted = await this.roleService.isPermitted('UniteReleve', 'edit');
        if (isPermitted) {
    ...
    set editDialog(value: boolean) { this.service.editDialog = value; }
