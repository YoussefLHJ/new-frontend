...
export class AppareilListAdminComponent implements OnInit {
    ...
    public findPaginatedByCriteria() {
        this.service.findPaginatedByCriteria(this.criteria).subscribe(paginatedItems => {
            ...
            this.selections = new Array<AppareilDto>();
    ...
    public async edit(dto: AppareilDto) {
    ...
    public async view(dto: AppareilDto) {
        this.service.findByIdWithAssociatedList(dto).subscribe(res => {
            this.item = res;
	...
	public async openEdit(dto: AppareilDto) {
        this.service.findByIdWithAssociatedList(dto).subscribe(res => {
    ...
    public async duplicate(dto: AppareilDto) {
    ...
    public exportPdf(dto: AppareilDto): void {
...
// add
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'dernierePeriodeIndex', header: 'Derniere periode index'},
            {field: 'derniereAnneeIndex', header: 'Derniere annee index'},
            {field: 'batiment?.libelle', header: 'Batiment'},
        ];
    }


    public async loadBatiment(){
        this.batimentService.findAllOptimized().subscribe(batiments => this.batiments = batiments, error => console.log(error))
    }


	public initDuplicate(res: AppareilDto) {
	}
        ...
        this.service.findByCriteria(this.criteria).subscribe(
            (allItems) =>{
                this.exportData = allItems.map(e => {
					return {
						'Numero': e.numero ,
						'Derniere periode index': e.dernierePeriodeIndex ,
						'Derniere annee index': e.derniereAnneeIndex ,
						'Batiment': e.batiment?.libelle ,
					}
				});

            this.criteriaData = [{
                'Numero': this.criteria.numero ? this.criteria.numero : environment.emptyForExport ,
                'Derniere periode index Min': this.criteria.dernierePeriodeIndexMin ? this.criteria.dernierePeriodeIndexMin : environment.emptyForExport ,
                'Derniere periode index Max': this.criteria.dernierePeriodeIndexMax ? this.criteria.dernierePeriodeIndexMax : environment.emptyForExport ,
                'Derniere annee index Min': this.criteria.derniereAnneeIndexMin ? this.criteria.derniereAnneeIndexMin : environment.emptyForExport ,
                'Derniere annee index Max': this.criteria.derniereAnneeIndexMax ? this.criteria.derniereAnneeIndexMax : environment.emptyForExport ,
            //'Batiment': this.criteria.batiment?.libelle ? this.criteria.batiment?.libelle : environment.emptyForExport ,
            }];
			}
    ...
    get selections(): Array<AppareilDto> {
        return this.service.selections;
    }

    set selections(value: Array<AppareilDto>) {
